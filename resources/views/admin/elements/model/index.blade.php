                                 ...
                                 <div class="form-group">
                                    ...
                                    <select class="form-control" name="modal">
                                       ...
                                       <option {{request()->input('modal') == $modals->modal_id ? 'selected':''}}  
                                          value="{{$modals->modal_id}}">{{$modals->modal_name}}-{{@$modals->category['model_cat_name']}}</option>
                                       @endforeach
                                    </select>
                                 </div>
                              </div>
               ...
               <table id="example" class="table table-striped table-bordered text-nowrap w-100">
                  <thead>
                     <tr>
                        ...
                        <th style="width:60px;" class="wd-15p">{{ __('Name') }}</th>
                        <!--<th style="width:60px;" class="wd-15p">{{ __('Name (Arabic)') }}</th>-->
                       {{-- <th class="wd-15p">{{ __('Category') }}</th>--}}
                        ...
                        <th class="wd-15p">{{ __('Status') }}</th>
                         <th class="wd-15p">{{ __('Availabitlity') }}</th>
                  ...
                  <tr>
                     <td>{{$i}}</td>
                     <td>{{ucfirst($modDetails->modal_name)}}
                     <br>
                     <p>{{ucfirst($modDetails->ar_modal_name)}}</p>
                     ({{ucfirst(@$modDetails->category['model_cat_name'])}})
                     </td>
                     <!--<td style="text-align: right; direction: rtl;">{{ucfirst($modDetails->ar_modal_name)}}</td>-->
                     {{--<td>{{ucfirst(@$modDetails->category['model_cat_name'])}}</td>--}}
                     <td>{{ucfirst(@$modDetails->maker['maker_name'])}}</td>
                     <td>
                        @php
                        $mRates = Mode_rate::where('model_id','=',$modDetails->model_number)->where('maker_id','=',$modDetails->makers)->orderBy('rate_type_id','ASC')->get();
                        @endphp
                        @if(!$mRates->isEmpty())
                        @php
                        //dd($mRates);
                        ...
                        {{$mRatess->rates->rate_type_code}}-{{$mRatess->model_year}}({{$mRatess->makers['maker_name']}}):{{$mRatess->rate}}<br>
                        @endforeach
                        @else
                        No Rates 
                        @endif
                     ...
                     <td>@if($modDetails->rdy_count!="0.0")  <p style="color:green;"> Available </p> @else <p style="color:red;">Rented Out</p>  @endif</td>
                     <td>
                ...
                <div class="modal fade bs-example-modal-lg{{$modDetails->modal_id}}" tabindex="-1" role="dialog" aria-hidden="true">
                     <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                           ...
                           <div class="modal-body">
                              <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data" method="POST" action="{{route('image.save')}}">
                                 ...
                                 <div class="item form-group">
                                    <label class="col-form-label col-md-12 col-sm-12 label-align" for="car_image" style="font-size: 12px;">{{ __('Image (Press and hold Cntrl key to select multiple images, Preferred resolution 728*485 )')}} <span class="required">*</span>
                                    </label>
                           ...
                           <button type="submit" id="submitImage" class="btn btn-primary">{{ __('Upload') }}</button>
                  ...
                  <div class="modal fade bs-example-modal-lgsec{{$modDetails->modal_id}}" tabindex="-1" role="dialog" aria-hidden="true">
                     <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                           ...
                           <div class="modal-body">
                              <form id="demo-form3" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data" method="POST" action="{{route('admin.model.spec.save')}}">
                                 @csrf
                                 <input type="hidden" value="{{$modDetails->modal_id}}" name="modal_id">
                                 <div class="item form-group">
