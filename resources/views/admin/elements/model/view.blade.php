        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label class="form-label">{{ __('Model Name (Arabic)') }}</label>
                           <input type="text" class="form-control" readonly=""  dir="rtl" lang="ar"  value="@if($viewRes->ar_modal_name!=""){{$viewRes->ar_modal_name}}@else Not Specified @endif">
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <label class="form-label">{{ __('Model Category') }}</label>
                           <input type="text" class="form-control" readonly="" value="@if($viewRes->category->model_cat_name!=""){{$viewRes->category->model_cat_name}}@else Not Specified @endif">
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
              ...
              <div class="form-group">
                <label class="form-label">{{ __('Specifications') }}</label>
                           <ul>
                              ...
                              <li>{{$viewSpecs->specs['spec_name']}}</li>
                              @endforeach
                           </ul>
              </div>
            </div>
              ...
              <div class="form-group">
                <label class="form-label">{{ __('Images') }}</label>
                        @foreach($viewImage as $viewImages)
                           <img src="{{url('/assets/uploads/models/'.$viewImages->model_image)}}" width="150" height="100">
                           @endforeach
              </div>
            </div>
            ...
            <table class="table table-striped table-bordered text-nowrap w-100"  style="width:100%">
               ...
               <tbody>
                  @php
                  $viewRate   = Mode_rate::where('model_id','=',$viewRes->model_number)->where('maker_id','=',$viewRes->makers)->orderBy('rate_type_id','ASC')->get();
                  $i=0;
                  @endphp
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <a class="btn btn-danger" href="{{route('admin.models')}}">Back to Models</a>
                </center>
              </div>
            </div>
          </div>
