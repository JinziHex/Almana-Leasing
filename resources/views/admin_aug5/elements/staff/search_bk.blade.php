                  ...
                  <div class="col-md-4">
                     <div class="form-group">
                        <label>{{ __('Employee Code')}}</label>
                         <input type="text" id="employee_code"  name="employee_code"  class="form-control" value="@if($empCode){{$empCode}}@endif">
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="form-group">
                        <label>{{ __('Mobile Number')}}</label>
                         <input type="text" id="mobile_number"  name="mobile_number"  class="form-control" value="@if($mobileNumber){{$mobileNumber}}@endif">
                     </div>
                  </div>
                  <div class="col-md-4">
                     <div class="form-group">
                        <label>{{ __('Username')}}</label>
                        <input type="text" id="username"  name="user_name"  class="form-control" value="@if($usrName){{$usrName}}@endif">
                     </div>
                  </div>
                  <div class="col-md-6">
                     <div class="form-group">
                  ...
                  <tbody>
                     ...
                     <td>
                       ...
                       <a href="{{url('staff/inactivate/'.Crypt::encryptString($usrDetails->id))}}" style="font-size: 12px;padding: 0px 5px;" class="btn btn-sm btn-outline-success" onclick="return confirm('Do you want to make this staff inactive?');">{{ __('Active') }}</a> 
                        ...
                        <td>
                           <a href="{{url('staff/edit/'.Crypt::encryptString($usrDetails->id))}}" class="btn btn-sm btn-info"><i class="fa fa-pencil-square-o"></i>&nbsp;{{ __('Edit') }}</a>&nbsp;<a href="{{url('staff/delete/'.Crypt::encryptString($usrDetails->id))}}" class="btn btn-sm btn-danger" onclick="return confirm('Do you want to delete this staff?');"><i class="fa fa-trash-o"></i>&nbsp;{{ __('Delete') }}</a></td>
