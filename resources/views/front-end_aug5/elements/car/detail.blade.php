      ...
      <div class="container">
         <form action="{{route('user.personal.info.save')}}" method="POST" class="row">
            ...
            <div class="form-group col-md-6">
               ...
               <input id="" class="form-control" required="" name="first_name" type="text" placeholder="Enter First Name" @if(session()->get('locale')=='ar') dir="rtl" lang="ar" style=" unicode-bidi:bidi-override;direction: RTL; " @endif value="@if(Auth::guard('main_customer')->check()){{Auth::guard('main_customer')->user()->customer->cust_fname }}@endif">
            </div>
            <div class="form-group col-md-6">
               <label for="">{{__('Last Name')}} <sup>*</sup></label>
               <input id="" class="form-control" required=""  type="text" name="last_name" placeholder="Enter Last Name" @if(session()->get('locale')=='ar') dir="rtl" lang="ar" style=" unicode-bidi:bidi-override;direction: RTL; " @endif value="@if(Auth::guard('main_customer')->check()){{Auth::guard('main_customer')->user()->customer->cust_lname}}@endif">
            </div>
            <div class="form-group col-md-6">
               <label for="">{{__('Mobile Number')}} <sup>*</sup></label>
               <input id="" class="form-control" required=""  type="text" name="mobile_number" placeholder="Enter your Mobile Number" @if(session()->get('locale')=='ar') dir="rtl" lang="ar" style=" unicode-bidi:bidi-override;direction: RTL; " @endif value="@if(Auth::guard('main_customer')->check()){{Auth::guard('main_customer')->user()->customer->cust_mobile_number}}@endif">
            </div>
                      ...
                      <option value="">{{__('Please choose city')}}</option>
                       @if(Auth::guard('main_customer')->check())
                        ...
                        @php $cityId = Auth::guard('main_customer')->user()->customer->cust_city; @endphp
                           ...
                           <option value="{{$fetchCitys->city_id}}" {{ $fetchCitys->city_id == $cityId ? 'selected' : '' }}>{{__($fetchCitys->city_name)}}</option>
                           @endforeach
                           @endif
                        @else
                        @foreach($fetchCity as $fetchCitys)
