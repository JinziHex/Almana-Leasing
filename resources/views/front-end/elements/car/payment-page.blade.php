...
<div class="rental-wrapper car-list-pg car-single-pg car-summary-pg">
   <div class="filter-bar">
      <div class="container">
       <h2 style="margin: auto; margin-top: 10px; text-align: center; color: #000;">{{__('Booking Summary')}}</h2>
      </div>
   </div>
...
<form action="{{route('user.booking.save')}}" method="POST" id="confirmForm">
   @csrf
   @php
   ...
   <input type="hidden" value="{{$book_file}}" name="book_file">
   ...
   <input type="hidden" value="{{$custLicNumber}}" name="book_bill_cust_lic_number">
   <input type="hidden" value="{{$custLicDate}}" name="book_bill_cust_lic_date">
    <input type="hidden" value="{{$custLicCountry}}" name="book_bill_cust_lic_country">
   ...
   <div class="list-details row shadow" style="text-align: center;">
        <div class="col-12 Rental-Dtls">
          <h3>{{__('Booking Confirmation')}}</h3>
        </div>
        <div class="col-12 col-md-12">
          <div class="mrgn">
          <p class="head">{{__('Hello')}}, {{$fName}}&nbsp; {{$lName}}</p>
          <!-- <p class="txt">Doha</p> -->
          </div>
          <div class="mrgn">
          <!-- <p class="head">Location</p> -->
          <p class="txt">{{__('Billing Address')}}:{{$address1}}<br>{{$address2}}</p>
            <p class="txt">{{__('Location')}}: {{$cityName}},&nbsp; {{$lcationName}},&nbsp; {{$custZipcode}},&nbsp; QATAR</p>
          <p class="txt">{{__('Mobile Number')}}: {{$MobileNumber}}<br>{{__('Passport Number')}}:{{$custPassNumber}}<br>{{__('Qatar Id')}} : {{$custQatarId}}
          <br>
          {{__('License Number')}} : {{$custLicNumber}}
          </p>
 ...
 <div class="row summary-date">
  <div class="col-12">
    <div class="col-12">
      <a href="#" class="btn btn-lg btn-block">{{__('Avaliable Coupons')}}</a>
    </div>
  ...
  @foreach($coupons as $coupon)
				<div class="card">
					<div class="card-header" id="heading{{$coupon->id}}">
						<h2 class="clearfix mb-0">
							...
							<input type="hidden" id="myInput" value="{{$coupon->coupon_code}}">
              ...
              {{__('From')}} :{{$coupon->minimum_order_amount}}	QAR	
              	<a role="button" style="border-style: dotted;" class="btn btn-sm btn-outilne-success" onclick="myFunction('{{$coupon->coupon_code}}')" ><i><u>{{__('Click Here To Copy Coupon')}}</u></i></a>
						</h2>
  ...
  @endforeach
        ...
        <div class="col-6 ">
          ...
          <div class="city">{{__(Session::get('location_name'))}}</div>
        </div>

        <div class="border-bottom"> </div>
        <div class="row bill-list">
          <div class="col-6 titl"><p>{{__('Daily Rate')}}:</p></div>
          <div class="col-6"><p>{{$rate_per_day}} {{Session::get('cur_code')}}</p></div>
        </div>
        <div class="row bill-list">
          <div class="col-6 titl"><p>{{__('No. of Days')}}</p></div>
          <div class="col-6"><p>{{Session::get('days')}} {{__('Days')}}</p></div>
        </div>
        <div class="row bill-list">
          <div class="col-6 titl"><p>{{__('Drop Fee')}}</p></div>
          <div class="col-6"><p>0.00 QAR</p></div>
        </div>
        <div class="row bill-list">
          <div class="col-6 titl"><p>{{__('Additional Package')}}</p></div>
          <div class="col-6"><p>0.00 QAR</p></div>
        </div>
        <div class="row bill-list">
          <div class="col-6 titl"><p>{{__('Apply Coupon Code')}}</p></div>
          <div class="col-3"><p><input type="text" id="coupon_code"  class="form-control"></p></div>

          <div class="col-3"><input type="hidden" id="tot_amt" value="{{$total_rate}}">
          <p>
          <input id="submitCoupon" type="button" class="btn btn-sm btn-danger"  class="form-control" value="{{__('APPLY')}}"><input id="removeCoupon" type="button" class="btn btn-sm btn-danger"  class="form-control" value="{{__('REMOVE COUPON')}}">
          </p></div>
        </div>
        <div class="row bill-list">
        <div class="col-6"></div>
        ...
        <div class="row bill-list summary-total">
          <div class="col-6 titl"><p>{{__('Total')}}:</p></div>
          <div class="col-6"><p id="rateTag">{{$total_rate}}</p> {{Session::get('cur_code')}}</p></div>
          @if($fetchModel->rdy_count!="0.0")
          <button type="submit" id="confirmBtn" class="btn btn-warning btn-lg btn-block mx-auto text-white">{{__('Confirm And Pay')}}</button>
          @else
           <button type="submit" disabled class="btn btn-warning btn-lg btn-block mx-auto text-white">{{__('Rented Out')}}</button>
          @endif
        </div>
...
function myFunction(text){
    alert('Coupon '+text+' copied to clipboard');
    var copyText = text;
    ...
    e.preventDefault();
    $("#confirmBtn").prop("disabled", true); //disable
    $("#confirmBtn").html('Please Wait...'); //disable
    $("#confirmForm").submit();
  ...
  var checkboxes = $("input[type='checkbox']"),
    ...
    submitButt.attr("disabled", !checkboxes.is(":checked"));
     ...
     $("#removeCoupon").click(function() {
        ...
        $.ajax({
          type: "GET",
          url:"{{url('/remove-coupon')}}?coupon_code="+coupon_code+"&&total_amount="+tot_amt,
          success: function(data) {
            //alert(data.status);
            $('#rateTag').text(data.total_rate); 
