...
.filterstyle button:hover{background-color: #e61e26 !important;}
.filterstyle .sortarrow:hover select{color:#fff;}
.filterstyle .sortarrow:hover label{color:#fff;}
/*.filterstyle select option:checked {*/
/*    text-align: center;*/
/*}*/
...
<div class="rental-wrapper car-list-pg">
  <div class="filter-bar">
    <div class="container sort_mob_style ">
      <div class="dropdown Choose no-wid">
        ...
        <form method="GET" action="{{route('car.search')}}">
          ...
          <div class="sortarrow">
            ...
            <select name="id"> {{-- onchange='if(this.value != 0) { this.form.submit(); }' --}}
              ...
              <option value="{{$vehicleTypes->model_cat_id}}">{{__($vehicleTypes->model_cat_name)}}</option>
              @endforeach
            </select>
          </div>
          <div class="sortarrow">
            ...
            <select name="bid">
              ...
              <option value="{{$vehicleMakers->maker_id}}">{{__($vehicleMakers->maker_name)}}</option>
              @endforeach
            </select>
          </div>
      ...
      <div class="dropdown Choose sort pos_drp_style">
  ...
  <form action="{{route('car.search')}}" method="GET" style="padding: 0 15px;">
    ...
    <div class="list-details row shadow">
      <div class="col-6 col-md-3">
        ...
        <div class="mrgn">
          ...
          <select class="form-control txt inp-select" id="SelectLocation" name="city_loc_id" required="">
        @if($cityLocations)
             @foreach($cityLocations as $loc)
              <option value="{{$loc->city_loc_id}}"   {{ $loc->city_loc_id == $location_id ? 'selected' : '' }}>{{__($loc->location_name)}}</option>
              @endforeach
        @endif
    ...
    <section class="container">
        <div id="myCarousel" class="carousel slide" data-ride="carousel">
    		<div class="carousel-inner" role="listbox">
    			...
    			<div class="adImage-box {{$key == 0 ? 'active' : ''}} item">
    				<img style="object-fit:cover;" src="{{url('assets/uploads/ads/'.$ad->image)}}">
    			</div>
    			@endforeach
    		</div>
    	</div>
    ...
    <div class="container">
      <div class="row">
        ...
        $rpDay = number_format($Modelss['Rate_per_day'], 2, '.', '');
        ...
        <div class="col-12 col-md-4">
          <div class="car-box1">
            <form action="{{ route('user.personal.info') }}" method="POST" style="height: 100%;position: relative;padding-bottom:199px;">
              ...
              <div class="titl">
                <h2>{{__($Modelss['Maker_name'])}} {{__($Modelss['Model_name'])}}  <!--<small><i>{{__('or similar')}}</i></small>--></h2>
            ...
            @foreach($mspecs as $specs)
                ...
                <p><img src="{{url('/assets/uploads/specifications/icons/'.$specification->spec_icon)}}" style="width: 25px !important;">  {{__($specification->spec_name)}}</p>
                @endif
            @endforeach
              </div>
              <button id="specMorBtn" style="display:none;" class="specMorBtn" type="button"><i class="fa fa-plus" aria-hidden="true" ></i>{{__('View more')}}</button>
              <div class="totaldv" style="position: absolute;width: 100%;bottom: 0;">
                <div class="car-price row">
                  <div class="col-6 day-price">
                    ...
                    @endif
                    <p class=""><small>{{__('Per Day')}}</small>{{number_format($Modelss['Rate_per_day'], 2, '.', '')}} &nbsp; {{Session::get('cur_code')}}</p>
                  </div>
                  <div class="col-6 totl-price ">
                    @if($Modelss['Offer_Rate']<$Modelss['Main_Rate'])
                   
                    <p class="ofr-price offer-new-price"><span><strike>{{$Modelss['Main_Rate']}} {{Session::get('cur_code')}}</strike></span>
                    <span>{{$Modelss['Offer_Rate']}} {{Session::get('cur_code')}}</span>
                    </p>
                    @else
                    <p class="ofr-price">{{$Modelss['Main_Rate']}} {{Session::get('cur_code')}}</p>
                    @endif
                ...
                @if(Auth::guard('main_customer')->check())
                 @if($Modelss['Model_available']!="0.0")
                <input type="submit" class="btn btn-warning btn-lg btn-block mx-auto text-white" style="margin-bottom: 10px;" value="{{__('Book Now')}}" >
                @else
                 <input type="submit" disabled class="btn btn-warning btn-lg btn-block mx-auto text-white rentout" style="margin-bottom: 10px;" value="{{__('Rented Out')}}" >
                @endif
                @else
                @if($Modelss['Model_available']!="0.0")
                <a href="{{route('user.login')}}" class="btn btn-warning btn-lg btn-block mx-auto text-white" style="margin-bottom: 10px;">{{__('Book Now')}}</a>
                @else
                  <a  class="btn btn-warning btn-lg btn-block mx-auto text-white rentout" style="margin-bottom: 10px;">{{__('Rented Out')}}</a>
                @endif
                @endif
              </div>
...
$("#SelectLocation").append('<option value="">{{__('Select a location')}}</option>');
if(locale=='en')
			{
				//alert('1');
