...
<div class="rental-wrapper car-list-pg yellowbar">
   ...
   <input type="hidden" name="customer_id" value="{{$custId}}">
         <div class="form-group col-md-6 time-picker">
            <label for="SelectCity">{{ __('First Name')}}</label>
             <input id="feedback_fname" type="text" value="{{Auth::guard('main_customer')->user()->customer->cust_fname}}" class="form-control" name="feedback_fname" required readonly>
         ...
         <div class="form-group col-md-6 time-picker">
            <label for="SelectLocation">{{ __('Last Name')}}</label>
             <input id="feedback_fname" type="text" value="{{Auth::guard('main_customer')->user()->customer->cust_lname}}" class="form-control" name="feedback_lname" required readonly>
         ...
         <div class="form-group col-md-6 date" id="sandbox-container">
          ...
          <input id="feedback_fname" type="email" value="{{Auth::guard('main_customer')->user()->email}}" class="form-control" name="feedback_email" required readonly>
         </div>
   ...
   <button type="submit" form="vehicle-search" class="btn btn-warning btn-lg btn-block mx-auto text-white" style="margin-top: 70px;">{{__('Submit')}} </button>
   </div>
   </form>
</div>
</div>
