...
<style>
  ...
  .mobilecode .select2-container .select2-selection {
  ...
  #select2-Nationality-container.focusdiv {
    ...
    <div class="col-md-12">
      ...
      <form action="{{route('user.save')}}" method="POST" id="login-form" class="row mx-0">
        ...
        <div class="form-group col-md-6 mobilecode">
          ...
          <input type="text" class="form-control" id="usermobile" placeholder="{{__('Enter Your')}} {{__('Mobile Number')}}" name="cust_mobile_number"  style="text-align: left;
padding-left: 95px;" required>
          <select class="form-control select2 col-md-3" style="margin-top: -33px;" name="cust_mobile_code" id="mobCode" required="">
            ...
            <option value="+{{$fetchContmc->phonecode}}" {{ $fetchContmc->country_id == "174" ? 'selected' : '' }}>+{{$fetchContmc->phonecode}}&nbsp;({{ @$fetchContmc->iso3 }})</option>
        ...
        <div class="form-group col-md-6">
          ...
          <input type="text" class="form-control" id="QatarID" name="cust_qatar_id" value="{{old('cust_qatar_id')}}" placeholder="{{__('Qatar ID')}}">
          <span id="errQatar"></span>
        </div>

        <div class="form-group col-md-6">
          <label for="PassportNumber">{{__('Passport Number')}}</label>
          <input type="text" class="form-control" id="PassportNumber" name="cust_passport_number" placeholder="{{__('Enter Your')}} {{__('Passport Number')}}" value="{{ old('cust_passport_number')}}">
        </div>
        ...
        <div class="form-group col-md-6 selectntn ">
          ...
          <select class="form-control select2 no-pos_select-style" id="Nationality" required name="cust_nationality" val {{ old('cust_nationality')}}>
  ...
  $(document).ready(function() {
    ...
    //qatar id validation
     $('#QatarID').on("keyup", function(){
        var qatar_id = $('#QatarID').val();
         if(qatar_id.length > 0){
             if(qatar_id.length == 11){
                $("#errQatar").css("display", "none");
             }else{
                $("#errQatar").html('The Customer Qatar Id should be 11 characters.').css('color', 'red').css('font-size', '9px').css('letter-spacing', '.5px');
                $("#errQatar").css("display", "");
            }
         }else{
             $("#errQatar").css("display", "none");
         }
    ...
    // $('#QatarID').bind("keyup change", function() {
    //   var length = 11;
    //   var value = $(this).val();
