...
<div class="rental-wrapper car-list-pg yellowbar">
   ...
   <input type="hidden" name="customer_id" value="{{$fetchCust->customer_id}}">
         ...
         <div class="form-group col-md-4 date time-picker">
           ...
           <select class="form-control" name="cust_bill_country" readonly>
               @if($fetchCust->customer['cust_bill_country'] != "")
             @foreach($getCont as $getConts)
             @if($getConts->country_id == '174')
            <option value="{{$getConts->country_id}}" {{ $getConts->country_id == '174' ? 'selected' : '' }}>{{$getConts->country_name}}</option>
            @endif
            @endforeach
            @else
         ...
         <div class="form-group col-md-4 date time-picker">
          ...
          <div class="form-group col-md-4 date time-picker">
            <label for="ToDate">{{ __('License Expiry Date') }}</label>
...
<script type="text/javascript">
    ...
    $('#cust_license_issued_date').attr('min', minDate);
